...
const FinalCTA = () => {
  ...
  const schema = z.object({
    fullName: z.string().trim().min(2, t("final_cta.val_name")).max(100),
    company: z.string().trim().min(2, t("final_cta.val_company")).max(200),
    whatsapp: z.string().trim().min(7, t("final_cta.val_whatsapp")).max(20),
    painPoint: z.string().trim().min(5, t("final_cta.val_pain")).max(1000),
  });
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      toast({
        title: t("final_cta.error_title"),
        description: t("final_cta.error_desc"),
        variant: "destructive",
      });
    } finally {
    ...
    >
      <div className="container mx-auto max-w-4xl">
        ...
        <div className="text-center mb-12 scroll-reveal space-y-4">
          <h2>
            {t("final_cta.title")}{" "}
            <span className="text-primary">{t("final_cta.title_accent")}</span>
          </h2>
        ...
        {!submitted && (
          ...
          >
            <p className="text-center text-sm mb-6 whitespace-pre-line" style={{ color: "#888888" }}>
              {t("final_cta.form_preamble")}
            </p>
              ...
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel style={{ color: "#888888", fontSize: "0.8rem" }}>{t("final_cta.full_name")}</FormLabel>
                        <FormControl>
                          <Input
                            placeholder={t("final_cta.full_name_placeholder")}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel style={{ color: "#888888", fontSize: "0.8rem" }}>{t("final_cta.company")}</FormLabel>
                        <FormControl>
                          <Input
                            placeholder={t("final_cta.company_placeholder")}
                            className="h-12 rounded-lg focus-visible:ring-primary"
                            style={{ background: "#0D0D0D", borderColor: "#333333" }}
                            {...field}
                          />
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel style={{ color: "#888888", fontSize: "0.8rem" }}>{t("final_cta.whatsapp")}</FormLabel>
                      <FormControl>
                        <Input
                          type="tel"
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel style={{ color: "#888888", fontSize: "0.8rem" }}>
                        {t("final_cta.pain_point_label")}
                      </FormLabel>
                      <FormControl>
                        <Textarea
                          placeholder={t("final_cta.pain_point_placeholder")}
                ...
                >
                  {isSubmitting ? t("final_cta.submitting") : t("final_cta.submit")}
                </Button>
              </form>
            </Form>
          </div>
        ...
        {submitted && !showCalendly && (
          <div className="rounded-2xl p-6 mb-8 flex items-center gap-4 animate-fade-in" style={{ background: "rgba(0,194,255,0.1)", border: "1px solid rgba(0,194,255,0.3)" }}>
            ...
            <p className="text-foreground font-medium">
              {t("final_cta.success", { name: userName })}
            </p>
          </div>
        )}
        ...
        {!submitted && (
          <div className="scroll-reveal rounded-2xl overflow-hidden bg-card border border-border opacity-30 pointer-events-none" style={{ transitionDelay: "0.2s" }}>
            <div className="flex items-center justify-center py-16 text-muted-foreground text-sm">
              <span>{t("final_cta.calendly_empty")}</span>
            </div>
          </div>
        )}
        ...
        <div className="scroll-reveal text-center mt-10 space-y-2 text-sm text-muted-foreground" style={{ transitionDelay: "0.3s" }}>
          <p>📍 David Shaw · GoFlow AI · {t("final_cta.contact_location")}</p>
          <p>🇪🇨 {t("final_cta.contact_ecuador")}</p>
          <p>🇦🇺 {t("final_cta.contact_australia")}</p>
        </div>
      </div>
    </section>
  );
